...
# ============================================================
    ...
    <script>
    (function() {{
        var steps = {json.dumps([(s, p) for s, p in _loading_steps])};
        var bar = document.getElementById('loadingBarFill');
        if (!bar) return;
        var cur = 0;
        steps.forEach(function(item) {{
            var pct = item[1];
            var delay = (pct - cur) * 30;
            cur = pct;
            setTimeout(function() {{ bar.style.width = pct + '%'; }}, delay);
        }});
    }})();
    </script>
    ''', unsafe_allow_html=True)
...
# ============================================================
    ...
    <div class="loading-overlay">
        <div class="loading-card">
            ...
            <div class="loading-bar-track">
                <div class="loading-bar-fill" id="promptLoadingBar" style="width:0%;"></div>
            </div>
        </div>
    </div>
    <script>
    (function() {{
        var steps = {json.dumps([(s, p) for s, p in _prompt_loading_steps])};
        var bar = document.getElementById('promptLoadingBar');
        if (!bar) return;
        var cur = 0;
        steps.forEach(function(item) {{
            var pct = item[1];
    ...
    # 直接将提示词文本写入session_state中的widget key，确保rerun后text_area能读到
    st.session_state["edit_prompt_area"] = prompt_result.get("prompt_text", "")
    # 保存提示词步骤快照
...
# ============================================================
    ...
    # 可编辑的提示词（key与session_state同步，生成时已预写入值，无需value参数）
    prompt_text = prompt_result.get("prompt_text", "")
    # 确保session_state中有初始值（兼容直接刷新页面的情况）
    if "edit_prompt_area" not in st.session_state:
        st.session_state["edit_prompt_area"] = prompt_text
