...
def _reset_all_state():
    ...
    for key in ["mode", "starfire", "member_framework",
                ...
                "selected_inspirations", "edit_prompt_area"]:
...
# ============================================================
    ...
    st.caption(f"{mode_icons.get(_mode, '')} 模式：**{_mode}**")
    ...
    if _mode == "流星":
        ...
        if _meteor_recommended:
            _meteor_display_text = _meteor_recommended[0]["title"] + "\n" + _meteor_recommended[0]["content"]
        else:
            _meteor_display_text = _inspiration_items[-1]["title"] + "\n" + _inspiration_items[-1]["content"] if _inspiration_items else inspiration_text
        ...
        # 非流星模式：显示完整灵感结果
        st.markdown(f'<div class="result-box">{_html_escape(inspiration_text)}</div>', unsafe_allow_html=True)
    ...
    if _is_meteor:
        ...
        selected_indices = list(range(len(_inspiration_items))) if _inspiration_items else []
        _chosen_inspiration = _inspiration_items[0]["title"] + "\n" + _inspiration_items[0]["content"] if _inspiration_items else inspiration_text
    elif _mode == "星图":
        # 星图模式：3选1
        ...
        # 引星模式：3条灵感，选1条
        st.markdown("#### 选择灵感")
        st.caption("从3条灵感中选择1条最满意的")

        if len(_inspiration_items) <= 1:
            selected_indices = list(range(len(_inspiration_items)))
            _chosen_inspiration = _inspiration_items[0]["title"] + "\n" + _inspiration_items[0]["content"] if _inspiration_items else inspiration_text
        else:
            selected_idx = st.radio(
                "选择1条灵感",
...
# ============================================================
    ...
    # 清除text_area缓存，确保新提示词能正确显示（Streamlit key缓存会忽略value参数）
    st.session_state.pop("edit_prompt_area", None)
    # 保存提示词步骤快照
...
# ============================================================
    ...
    # 复制按钮
    _copy_id = "copy_prompt_final"
    _escaped = _html_escape(new_prompt)
    st.markdown(f"""
    <textarea id="{_copy_id}" style="position:fixed;left:-9999px;">{_escaped}</textarea>
    <button onclick="var ta=document.getElementById('{_copy_id}');ta.select();ta.setSelectionRange(0,99999);document.execCommand('copy');this.textContent='已复制!';var b=this;setTimeout(function(){{b.textContent='复制提示词';}},1500);" 
        style="background:rgba(100,150,220,0.15);border:1px solid rgba(120,160,220,0.25);border-radius:6px;color:#a0c0f0;padding:4px 12px;font-size:0.8rem;cursor:pointer;margin:4px 0;">
        复制提示词
    </button>
    """, unsafe_allow_html=True)
    ...
    with col1:
        if st.button("← 返回修改灵感", use_container_width=True):
            st.session_state.generated_prompt = None
            st.session_state.planning_saved = False
            st.session_state.step = 3
            st.rerun()
    with col2:
