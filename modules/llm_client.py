...
_RINGTONE_BOUNDARY = _build_ringtone_boundary()

def _build_system_prompt_base():
    """动态构建系统提示词，规避规则从暗礁库读取"""
    avoidance = _get_avoidance_hint()
    if avoidance:
...
_SYSTEM_PROMPT_BASE = _build_system_prompt_base()
...
只输出这3条，不要输出其他任何内容。"""
    ...
    system_prompt = _SYSTEM_PROMPT_BASE + "你尊重成员的创作方向，只补充不改变。"
...
只输出这3条，不要输出其他任何内容。"""
    ...
    system_prompt = _SYSTEM_PROMPT_BASE + "你必须先精准判断哪些维度最匹配用户需求，再针对这些维度生成联想。你选出的3个维度必须彼此差异最大，覆盖不同风格方向。你的联想必须具体、有画面感、可转化为视觉表达。"
...
只输出以上内容，不要输出其他任何内容。"""
    ...
    system_prompt = _SYSTEM_PROMPT_BASE + "你擅长在氛围引导下自由联想。你的联想有画面感、有意境、可转化为视觉表达。你会在所有联想中精准选出最匹配用户需求的那一条。"
